...
void Engine::Render::Collider() {
	...
	Engine::Render::gpu->bind(Engine::Shaders::collider);
	...
	Engine::Render::gpu->autoPixelCBuffer(colliderColor);
	...
	case Engine::Collider::Shape::Sphere:
	{
		...
		Engine::Render::gpu->autoVertexCBuffer(wvp);
		Engine::Render::gpu->draw(Engine::Default::sphere->buffer);
	}
	break;
	...
	case Engine::Collider::Shape::Capsule:
	{
		wvp *= kl::mat4::scale(Engine::Selected::entity->collider.scale);
		Engine::Render::gpu->autoVertexCBuffer(wvp);
		Engine::Render::gpu->draw(Engine::Default::capsule->buffer);
	}
	break;
	...
	case Engine::Collider::Shape::Mesh:
	{
		wvp *= kl::mat4::scale(Engine::Selected::entity->scale * Engine::Selected::entity->collider.scale);
		Engine::Render::gpu->autoVertexCBuffer(wvp);
		Engine::Render::gpu->draw(Engine::Selected::entity->mesh->buffer);
	}
	break;
	}
}
