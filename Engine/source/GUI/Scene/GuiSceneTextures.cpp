...
void Engine::GUI::Scene::Textures() {
				...
				if (!Engine::gameRunning && ImGui::Button("Delete", ImVec2(winWidth, 0.0f))) {
					for (auto& ent : Engine::entities) {
						if (ent->material.colorMap == tex) {
							ent->material.colorMap = Engine::Textures::Default::colorMap;
						}
						if (ent->material.normalMap == tex) {
							ent->material.normalMap = Engine::Textures::Default::nullMap;
						}
						if (ent->material.roughnessMap == tex) {
							ent->material.roughnessMap = Engine::Textures::Default::nullMap;
						}
					}
