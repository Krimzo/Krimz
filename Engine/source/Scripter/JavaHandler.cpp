...
void Krimz::JavaHandler::Init()
{
	...
	std::string classPaths[2] = { ".", "../ScriptApi/ScriptApi.jar" };
	std::string jvmOptionsData = kl::format(
		"-Djava.class.path=",
		classPaths[0], ";",
		classPaths[1], ";"
	);
	...
	kl::console::error(JNI_CreateJavaVM(&jvm, (void**) &env, &vmArgs), "Failed to create JVM");
...
jclass Krimz::JavaHandler::LoadClass(const std::string& filePath)
{
	...
	if (!clsDef)
	{
		...
		Krimz::log("Could not define class \"" + filePath + "\"!");
		return nullptr;
	}
